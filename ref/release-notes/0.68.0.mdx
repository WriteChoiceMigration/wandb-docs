...
- Registry admins can define and assign [*protected aliases*](/guides/core/registry/model_registry/access_controls#add-protected-aliases) to represent key stages of your development pipeline. A protected alias can be assigned only by a registry admin. W&B blocks other users from adding or removing protected aliases from versions in a registry using the API or UI.
- You can now filter console logs based on a run's `x_label` value. During [distributed training](/guides/models/track/log/distributed-training#track-all-processes-to-a-single-run), this optional parameter tracks the node that logged the run.
