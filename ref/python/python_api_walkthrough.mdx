...
Use the W&B Python SDK (`wandb.sdk`) to interact with W&B during training. Log the loss using [`wandb.Run.log()`](/ref/python/sdk/classes/run/#method-runlog), then save the trained model as an artifact using [`wandb.Artifact`](/ref/python/sdk/classes/artifact) before finally adding the model file using [`Artifact.add_file`](/ref/python/sdk/classes/artifact#add_file).
...
To use a model for inference, use `wandb.Run.use_artifact()` to retrieve the published artifact from the registry. This returns an artifact object that you can then use [`wandb.Artifact.download()`](/ref/python/sdk/classes/artifact/#method-artifactdownload) to download the artifact to a local file.
...
Create and share a [report](/guides/core/reports/_index) to summarize your work. To create a report programmatically, use the [W&B Report and Workspace API](/ref/python/wandb_workspaces/reports).
...
For more information on querying the registry, see the [Query registry items with MongoDB-style queries](/guides/core/registry/search_registry#query-registry-items-with-mongodb-style-queries).
