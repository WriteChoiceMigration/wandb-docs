...
This section outlines workflows for managing and editing Artifacts. Many workflows utilize [the W&B API](/guides/models/track/public-api-guide), a component of [the client library](/ref/python/) that provides access to W&B-stored data.
</Update>
...
For more information on `wandb.init()`, see the [`wandb.init()` reference](/ref/python/sdk/functions/init).
</Update>
...
Refer to the link for details on roles and permissions: [Team Roles and Permissions](/guides/models/app/settings-page/teams#team-roles-and-permissions).
</Update>
...
2. If the network connection is unstable, operate in offline mode by setting the [optional environment variable](/guides/models/track/environment-variables#optional-environment-variables) `WANDB_MODE` to `offline`, and sync files later from a device with Internet access.
...
A run can have multiple tags, so grouping by tags is not supported. Add a value to the [`config`](/guides/models/track/config) object for these runs and group by this config value instead. This can be accomplished using [the API](/guides/models/track/config#set-the-configuration-after-your-run-has-finished).
</Update>
...
A team admin can [remove you from a team](/guides/models/app/settings-page/teams)  from the **Users** tab of the team settings.
</Update>
...
Delete your user account by clicking **Delete account** in your [user settings](/guides/models/app/settings-page/user-settings#delete-your-account). Note that this action is irreversible and it takes effect immediately.
</Update>
...
For complex workflows, use multiple runs and set the group parameters in [`wandb.init()`](/guides/models/track/create-an-experiment) to a unique value for all processes within a single experiment. The [**Runs** tab](/guides/models/track/project-page#runs-tab) will group the table by group ID, ensuring that visualizations function properly. This approach enables concurrent experiments and training runs while logging results in one location.
...
1. Call `wandb.init` in all processes and use the [group](/guides/models/track/runs/grouping) keyword argument to create a shared group. Each process will have its own wandb run, and the UI will group the training processes together.
...
<Note>
Refer to the [Distributed Training Guide](/guides/models/track/log/distributed-training) for detailed explanations of these approaches, including code examples with Torch DDP.
</Note>
</Update>
...
In the [Workspace](/guides/models/track/project-page#workspace-tab) settings, adjust the grouping of panels based on either the first segment or all segments separated by `/`.
</Update>
...
Create a multi-line custom chart with `wandb.plot.line_series()`. Navigate to the [project page](/guides/models/track/project-page) to view the line chart. To add a legend, include the `keys` argument in `wandb.plot.line_series()`. For example:
...
Refer to additional details about multi-line plots [here](/guides/models/track/log/plots#basic-charts) under the **Multi-line** tab.
</Update>
...
1. Choose a new value for **Project visibility**:
    ...
    - **Public**: Anyone can view the project, but only your team can edit it.
        ...
        <Note>
        If your W&B admins have turned off **Public** visibility, you cannot choose it. Instead, you can share a view-only [W&B Report](/guides/core/reports/collaborate-on-reports#share-a-report), or contact your W&B organization's admins for assistance.
        </Note>
1. Click **Save**.
...
If a grid search completes but some W&B Runs need re-execution due to crashes, delete the specific W&B Runs to re-run. Then, select the **Resume** button on the [sweep control page](/guides/models/sweeps/sweeps-ui). Start new W&B Sweep agents using the new Sweep ID.
...
To manage artifacts that contain sensitive data or to schedule the deletion of artifact versions, set a TTL (time-to-live) policy. For detailed instructions, refer to the [TTL guide](/guides/core/artifacts/manage-data/ttl).
</Update>
...
<Note>
Apart from the **Built-in** service accounts, W&B also supports **External service accounts** using [identity federation for SDK and CLI](/guides/hosting/iam/authentication/identity_federation#external-service-accounts). Use external service accounts if you are looking to automate W&B tasks using service identities managed in your identity provider that can issue JSON Web Tokens (JWT).
</Note>
</Update>
...
Set the environment variable [`WANDB_SILENT`](/guides/models/track/environment-variables) to `true`.
...
For additional information about teams, visit the [teams section](/guides/models/app/settings-page/teams).
</Update>
...
Visit the [Team roles and permissions](/guides/models/app/settings-page/teams#team-roles-and-permissions) page for an overview of the available roles and permissions.
</Update>
