...
* [`wandb.init()`](/ref/python/sdk/functions/init/): Initialize a new W&B Run. Each run is a single execution of the training function.
* [`run.config`](/guides/models/track/config): Pass sweep configuration with the hyperparameters you want to experiment with.
* [`run.log()`](/ref/python/sdk/classes/run/#method-runlog): Log the training loss for each epoch.
