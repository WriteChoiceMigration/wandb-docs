...
그러면 Launch 에이전트가 지정된 컨테이너를 가리키는 작업을 시작합니다. 컨테이너 레지스트리에서 이미지를 가져오기 위해 에이전트 엑세스를 구성하는 방법에 대한 예는 [고급 에이전트 설정](/ko/launch/set-up-launch/setup-agent-advanced.md#agent-configuration)을 참조하세요.
