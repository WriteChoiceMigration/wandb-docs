...
스윕 구성은 중첩된 키-값 쌍으로 구성됩니다. 스윕 구성 내에서 최상위 키를 사용하여 검색할 파라미터 ( [`parameter`](/ko/./sweep-config-keys.md#parameters) 키), 파라미터 공간을 검색하는 방법 ( [`method`](/ko/./sweep-config-keys.md#method) 키) 등과 같은 스윕 검색의 특성을 정의합니다.
