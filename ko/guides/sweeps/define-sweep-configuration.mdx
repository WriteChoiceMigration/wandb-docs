...
스윕 구성 내에서 최상위 키를 사용하여 스윕 이름([`name`](/ko/./sweep-config-keys) 키), 검색할 파라미터([`parameters`](/ko/./sweep-config-keys.md#parameters) 키), 파라미터 공간을 검색하는 방법([`method`](/ko/./sweep-config-keys.md#method) 키) 등과 같은 스윕 검색의 품질을 정의합니다.
...
최상위 `parameters` 키 내에는 `learning_rate`, `batch_size`, `epoch` 및 `optimizer` 키가 중첩되어 있습니다. 중첩된 각 키에 대해 하나 이상의 값, 분포, 확률 등을 제공할 수 있습니다. 자세한 내용은 [스윕 구성 옵션](/ko/./sweep-config-keys)의 [파라미터](/ko/./sweep-config-keys.md#parameters) 섹션을 참조하십시오.
