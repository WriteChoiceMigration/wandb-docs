...
[W&B Registry App의 글로벌 검색 창](/ko/./search_registry.md#search-for-registry-items)을 사용하여 registry, collection, artifact version tag, collection tag 또는 에일리어스를 찾으세요. W&B Python SDK를 사용하여 특정 기준에 따라 [MongoDB 스타일 쿼리로 registries, collections 및 artifact versions을 필터링](/ko/./search_registry.md#query-registry-items-with-mongodb-style-queries)할 수 있습니다.
...
[`wandb.Api().registries()`](/ko/ref/python/public-api/api.md#registries) 및 [쿼리 predicates](https://www.mongodb.com/docs/manual/reference/glossary/#std-term-query-predicate)를 사용하여 하나 이상의 [MongoDB 스타일 쿼리](https://www.mongodb.com/docs/compass/current/query/filter/)를 기반으로 registries, collections 및 artifact versions을 필터링합니다.
