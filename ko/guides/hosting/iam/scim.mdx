...
-   **엔드포인트**: **`<host-url>/scim/Users/{id}`**
-   **메서드**: PATCH
-   **설명**: 사용자에게 organization 수준 역할을 할당합니다. 역할은 [여기](/ko/access-management/manage-organization.md#invite-a-user)에 설명된 대로 `admin`, `viewer` 또는 `member` 중 하나일 수 있습니다. [SaaS Cloud](/ko/guides/hosting/hosting-options/saas_cloud)의 경우 사용자 설정에서 SCIM API에 대한 올바른 organization을 구성했는지 확인합니다.
-   **지원되는 필드**:
...
-   **엔드포인트**: **`<host-url>/scim/Users/{id}`**
-   **메서드**: PATCH
-   **설명**: 사용자에게 팀 수준 역할을 할당합니다. 역할은 [여기](/ko/access-management/manage-organization.md#assign-or-update-a-team-members-role)에 설명된 대로 `admin`, `viewer`, `member` 또는 사용자 지정 역할 중 하나일 수 있습니다. [SaaS Cloud](/ko/guides/hosting/hosting-options/saas_cloud)의 경우 사용자 설정에서 SCIM API에 대한 올바른 organization을 구성했는지 확인합니다.
-   **지원되는 필드**:
