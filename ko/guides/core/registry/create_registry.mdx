...
<Info>
코어 및 사용자 정의 레지스트리에 대한 전체 비교는 [레지스트리 유형](/ko/registry_types.md#summary)의 요약 표를 참조하세요.
</Info>
...
5. **Registry visibility** 드롭다운에서 레지스트리를 볼 수 있는 사용자를 선택합니다. 레지스트리 visibility 옵션에 대한 자세한 내용은 [레지스트리 visibility types](/ko/./configure_registry.md#registry-visibility-types)을 참조하세요.
...
팀 관리자가 사용자 정의 레지스트리를 만들 때 visibility를 할당하는 방법에 대한 자세한 내용은 [사용자 정의 레지스트리 만들기](/ko/./create_registry.md#create-a-custom-registry)를 참조하세요.
