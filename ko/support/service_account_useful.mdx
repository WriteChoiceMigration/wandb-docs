...
자세한 내용은 [팀 서비스 계정 행동](/ko/guides/models/app/settings-page/teams.md#team-service-account-behavior)을 참조하십시오.
...
<Note>
**Built-in** 서비스 계정 외에도 W&B 는 [SDK 및 CLI 에 대한 ID 페더레이션](/ko/guides/hosting/iam/authentication/identity_federation.md#external-service-accounts)을 사용하는 **External service accounts** 도 지원합니다. JSON Web Tokens (JWT)를 발행할 수 있는 ID 공급자에서 관리되는 서비스 ID 를 사용하여 W&B 작업을 자동화하려는 경우 외부 서비스 계정을 사용하십시오.
</Note>
