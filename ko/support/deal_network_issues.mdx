...
2. 네트워크 연결이 불안정한 경우, [선택적 환경 변수](/ko/guides/models/track/environment-variables.md#optional-environment-variables) `WANDB_MODE`를 `offline`으로 설정하여 오프라인 모드로 작동하고, 인터넷 엑세스가 가능한 장치에서 나중에 파일을 동기화합니다.
