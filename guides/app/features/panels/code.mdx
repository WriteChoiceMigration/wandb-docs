...
This captures all python source code files in the current directory and all subdirectories as an [artifact](/ref/python/sdk/classes/artifact). For more control over the types and locations of source code files that are saved, see the [reference docs](/ref/python/sdk/classes/run#log_code).
