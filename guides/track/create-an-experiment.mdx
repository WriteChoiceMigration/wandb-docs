...
Use [`wandb.init()`](/ref/python/sdk/functions/init) to create a W&B Run.
...
`wandb.init()` returns a [Run](/ref/python/sdk/classes/run) object.
...
Call [`run.log()`](/ref/python/sdk/classes/run/#method-runlog) to log metrics about each training step such as accuracy and loss.
...
For more information about available parameters when defining a W&B Experiment, see the [`wandb.init()`](/ref/python/sdk/functions/init) API docs in the [API Reference Guide](/ref/python/).