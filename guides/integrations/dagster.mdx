...
<Tab title="Tab">
Any W&B object, such as a [Table](/ref/python/sdk/data-types/table) or [Image](/ref/python/sdk/data-types/image), is added to an Artifact created by the integration. This example adds a Table to an Artifact:
...
* [`add_dirs`](/ref/python/sdk/classes/artifact#add_dir): (list[dict[str, Any]]): An array containing configuration for each local directory to include in the Artifact. 
* [`add_files`](/ref/python/sdk/classes/artifact#add_file): (list[dict[str, Any]]): An array containing configuration for each local file to include in the Artifact. 
* [`add_references`](/ref/python/sdk/classes/artifact#add_reference): (list[dict[str, Any]]): An array containing configuration for each external reference to include in the Artifact. 
