...
| `init_kwargs`                   | Params to pass to `wandb.init()` such as your wandb `config` etc. See the [`wandb.init()` parameters](/ref/python/sdk/functions/init) for parameters that `wandb.init()` accepts.
