...
W&B stores runs that you create into [*projects*](/guides/models/track/project-page). You can view runs and their properties within the run's project workspace on the W&B App. You can also programmatically access run properties with the [`wandb.Api.Run`](/ref/python/sdk/classes/run) object.
...
Initialize a W&B Run with [`wandb.init()`](/ref/python/sdk/functions/init). The proceeding code snippet shows how to import the W&B Python SDK and initialize a run. 
...
### Custom run IDs
You can specify your own run ID by passing the `id` parameter to the [`wandb.init()`](/ref/python/sdk/functions/init) method. 
...
You can specify a name for your run by passing the `name` parameter to the [`wandb.init()`](/ref/python/sdk/functions/init) method. 
...
* **Git repository**: The git repository associated with the run. You must [enable git](/guides/models/app/settings-page/user-settings#personal-github-integration) to view this field.
