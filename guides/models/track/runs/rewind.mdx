...
* You must use monotonically increasing steps. This does not work with non-monotonic steps defined with [`define_metric()`](/ref/python/sdk/classes/run#define_metric) because it disrupts the required chronological order of run history and system metrics.
</Note>
...
Use `resume_from` with [`wandb.init()`](/ref/python/sdk/functions/init) to "rewind" a run’s history to a specific step. Specify the name of the run and the step you want to rewind from:
