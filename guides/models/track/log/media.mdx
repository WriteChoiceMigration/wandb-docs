...
<Note>
For details, see the [Data types reference](/ref/python/sdk/data-types/).
</Note>
...
Using this schema, you can define a Python object and pass it in to [the `from_point_cloud` method](/ref/python/sdk/data-types/Object3D/#from_point_cloud).
...
You can use [the `from_file` method](/ref/python/sdk/data-types/Object3D/#from_file) to load in a JSON file full of point cloud data.
...
Using [the same array formats defined above](#numpy-array-formats), you can use `numpy` arrays directly with [the `from_numpy` method](/ref/python/sdk/data-types/Object3D/#from_numpy) to define a point cloud.
...
[`wandb.Image`](/ref/python/sdk/data-types/image) converts `numpy` arrays or instances of `PILImage` to PNGs by default.
...
Videos are logged using the [`wandb.Video`](/ref/python/sdk/data-types/Video) data type:
...
You can log a 2D view of a molecule using the [`wandb.Image`](/ref/python/sdk/data-types/image) data type and [`rdkit`](https://www.rdkit.org/docs/index.html):
