...
Within the rank 0 process, initialize a W&B run with [`wandb.init()`](/ref/python/sdk/functions/init) and log experiments ([`wandb.log`](/ref/python/sdk/classes/run/#method-runlog)) to that run.
...
Within the primary node, initialize a W&B run with [`wandb.init()`](/ref/python/sdk/functions/init). Pass in a `wandb.Settings` object to the `settings` parameter (`wandb.init(settings=wandb.Settings()`) with the following:
...
For each worker node, initialize a W&B run with [`wandb.init()`](/ref/python/sdk/functions/init) and provide the following:
