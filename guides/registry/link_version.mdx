...
<Note>
The term "type" refers to the artifact object's type. When you create an artifact object ([`wandb.Artifact`](/ref/python/sdk/classes/artifact)), or log an artifact ([`wandb.init.log_artifact`](/ref/python/sdk/classes/run#log_artifact)), you specify a type for the `type` parameter. 
...
Programmatically link an artifact version to a collection with [`wandb.init.Run.link_artifact()`](/ref/python/sdk/classes/run#link_artifact).
