...
`wandb sweep` generates a sweep ID. After you initialized the sweep, start a controller with [`wandb controller`](/ref/python/sdk/functions/controller):
...
The simplest way to use a controller with the Python SDK is to pass the sweep ID to the [`wandb.controller`](/ref/python/sdk/functions/controller) method. Next, use the return objects `run` method to start the sweep job:
